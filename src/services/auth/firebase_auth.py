...
#
# During development:
#   FIREBASE_API_KEY can come from .env
#
# During production:
#   build_lyrx.bat temporarily generates _runtime_config.py
#   containing only the Firebase Web API key.
#
# .env is NOT required on the user's PC.
# ------------------------------------------------------------------
try:
    from ._runtime_config import (
        FIREBASE_API_KEY as BUNDLED_FIREBASE_API_KEY,
    )
except ImportError:
...
class FirebaseAuthService:
    ...
    def __init__(self):
        self.api_key = (
            str(BUNDLED_FIREBASE_API_KEY or "").strip()
            or os.getenv("FIREBASE_API_KEY", "").strip()
        )

        if not self.api_key:
            raise RuntimeError(
                "LYRx authentication configuration is missing."
            )
    ...
    def sign_in_with_google_id_token(
        ...
        url = (
            f"{self.base_url}/accounts:signInWithIdp"
            f"?key={self.api_key}"
        )
        ...
        if lookup.success and lookup.user:
            ...
            # Google identity providers normally return a
            # verified email.
    ...
    def lookup_account(
        ...
        if not users:
            return AuthResult(
                False,
                "Firebase user could not be found.",
            )
    ...
    def sign_in_with_phone_number(
        ...
        lookup = self.lookup_account(
            result.user.id_token
        )

        if lookup.success and lookup.user:
            lookup.user.refresh_token = (
                result.user.refresh_token
            )
            lookup.user.expires_in = (
                result.user.expires_in
            )
    ...
    def _friendly_error(
        ...
        friendly = {
            ...
            "SESSION_EXPIRED":
                "This OTP session expired. Request a new OTP.",

            "INVALID_SESSION_INFO":
                "This phone verification session is invalid. Request a new OTP.",

            "MISSING_SESSION_INFO":
                "Phone verification session is missing.",

            "QUOTA_EXCEEDED":
                "Firebase SMS quota has been exceeded. Try again later.",
